...
\usepackage[english]{varioref}			% Per usare il comando \vref{label}, che dà dei collegamenti più dettagliati
...
\section*{The concert Bayesian Network 1/2}
	\begin{frame}{The concert Bayesian Network}{A brief explaination}
		...
		\onslide<2->{The table~\ref{tab:BNsummary} lists the nodes in the 
		network, summarizing the essential interpretational aspects.}
		\smallskip 

		\onslide<3->{Let's highlight here a few key ideas (a full discussion 
		is in the \texttt{ConcertBN.py} file):}
		\begin{itemize}
	...
	\begin{frame}{The concert Bayesian Network 2/2}{Summarizing table}
		\begin{table}[ht]
			\caption{\emph{A summary of the concert Bayesian Network.}}% In the "Law" column:
			%B $\equiv$ Bernoullian r.v., C $\equiv$ categorical.}}
			\label{tab:BNsummary}
	...
	\begin{frame}{Visual representation of the Network}
		\begin{center}
			...
			Network. The doubly-circled nodes have a categorical distribution.}}
		\end{figure}
		\end{center}
	\end{frame}

	\begin{frame}{Sampling from the concert's network}
		{Result of the sampling, and some further comment}

		$10000$ samples have been drawn from the distributions of the 
		\texttt{bureaucracy} ($B$), \texttt{concert\_held} ($CH$) and 
		\texttt{concert\_success} ($Cs$) random variables. 
		\smallskip 

		\onslide<2->{The outcome was:
		\begin{gather*}
			\mathbb{P}\left(B=1\right)=\mathbb{P}\left(B=1\mid S,Sc,Ce\right) \approx 0.881 \\
			\mathbb{P}\left(CH=1\right)=\mathbb{P}\left(CH=1\mid B,D,W\right) \approx 0.821 \\
			\mathbb{P}\left(Cs=1\right)=\mathbb{P}\left(Cs=1\mid CH,G,M,Gpc\right) \approx 0.418 
		\end{gather*}}

		\onslide<3->{As it was expected, the concert is very likely to be authorized by the 
		bureaucracy, but the probability\footnote{According to the 
		samples, of course (that is, sampling gives an estimation of the true  marginal distribution)} 
		of the concert being held is lower than that of the concert
		being authorized.} \onslide<3->{This is because the drummer being sick
		could lead the organizers to cancel the concert (with an increasing 
		probability, according to how bad the weather is).} 

		%\begin{equation*}
		%	\qty(0.07+0.6\cdot B+D\cdot\qty(0.29\cdot B + 0.13\cdot \qty(1-B)))^{\frac{W+3}{4}}
		%\end{equation*}
	\end{frame}
...
\section*{Comments and conclusion}
    \begin{frame}{Conclusion, and comments}
		...
		\onslide<4->{One example from the concert Bayesian Network:
		the civil engineering office's response's dependency 
		from the weather is an irrealistic simplification. 
		More properly, it should depend on a weather forecast. 
